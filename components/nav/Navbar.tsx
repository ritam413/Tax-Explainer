...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Show navbar on initial page load for 3 seconds, then collapse into top green rectangle
  useEffect(() => {
  ...
  return (
    <>
      {/* Reduced-height Top Green Accent Rectangle Handle - Exclusive Trigger */}
      <div
        className={`fixed top-0 left-0 right-0 h-1 z-50 cursor-pointer transition-all duration-300 flex items-center justify-center ${
          shouldShow
            ? 'opacity-0 pointer-events-none'
            : 'opacity-100 bg-[var(--accent-pulse)] shadow-[0_0_8px_rgba(127,238,100,0.8)] hover:h-2'
        }`}
        onMouseEnter={() => setIsVisible(true)}
        title="Hover green rectangle to expand FiscalQuant navigation"
      >
        <div className="w-12 h-[2px] bg-black/40 rounded-full" />
      </div>

      {/* Collapsible Fixed Header */}
      <header
        ...
        className={`fixed top-0 left-0 right-0 z-40 w-full bg-[var(--bg-hover)]/95 backdrop-blur-md border-b border-[var(--border-subtle)] transition-all duration-300 ease-in-out transform ${
          shouldShow
      ...
      >
        <nav className="relative h-[64px] max-w-[1280px] mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-2 bg-[var(--bg-card)] border border-[var(--border-color)] px-3.5 py-1 rounded-full text-[12px] font-medium text-[var(--badge-text)] shadow-sm">
